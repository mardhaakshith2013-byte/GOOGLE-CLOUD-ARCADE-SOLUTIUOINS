...
echo -e "${BLUE}║      Welcome to Dr. Akshith's Lab          ║${NC}"
echo -e "${BOX_BOT}"
echo

# Prompt for Zone if environment variable is missing
if [ -z "$ZONE" ]; then
  read -p "$(echo -e "${YELLOW}Enter the Compute Zone (e.g., us-central1-a): ${NC}")" ZONE
  export ZONE=$ZONE
fi
...
# Install Terraform
echo -e "${MAGENTA}📦 Installing HashiCorp Terraform...${NC}"
...
echo -e "\n${YELLOW}🛠️ Phase 1: Deploying Network Infrastructure...${NC}"
cat > main.tf <<EOF
terraform {
  required_providers {
    google = {
...
echo -e "╚════════════════════════════════════════════╝${NC}"
echo -e "${WHITE}Thank you for using Dr. Akshith's Automated Cloud Lab!${NC}"
echo
